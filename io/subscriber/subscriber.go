...
func (sub *Subscriber) AddInterceptor(interceptors ...easypubsub.Interceptor) {
	sub.o.interceptors = append(sub.o.interceptors, interceptors...)
}
...
FirstLoop:
	for {
		...
		HandleMsg:
			...
			for _, interceptor := range sub.o.interceptors {
				err := interceptor(topic, msg)
				if err != nil {
					sub.errC <- err
					continue FirstLoop
				}
